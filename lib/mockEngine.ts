...
const SIGNALS: Array<{
...
}> = [
  {
    name: "Urgency",
    severity: "high",
    patterns: [/right now/i, /immediately/i, /within \d+ (minutes|hours)/i, /before it'?s too late/i, /hurry/i, /act now/i],
  ...
  },
  {
    ...
    severity: "high",
    patterns: [/this is your (bank|grandson|grandma|son|daughter)/i, /from the (irs|bank|police|government|court)/i, /officer\b/i, /attorney/i, /it'?s me/i],
  ...
  },
  {
    name: "Secrecy",
    severity: "high",
    patterns: [/don'?t tell (anyone|mom|dad|your family)/i, /keep this (between us|a secret)/i, /just between you and me/i],
  ...
  },
  {
    name: "Fear / Threat",
    severity: "high",
    patterns: [/arrest(ed)?/i, /jail/i, /lawsuit|sued|legal action/i, /account (will be )?(closed|suspended|frozen)/i, /deport/i, /warrant/i],
  ...
  },
  {
    ...
    severity: "high",
    patterns: [/gift card/i, /wire (transfer|the money)/i, /bitcoin|crypto|usdt/i, /western union|moneygram/i, /zelle|cash app|venmo/i, /bail/i],
  ...
  },
  {
    ...
    severity: "medium",
    patterns: [/don'?t hang up/i, /stay on the (line|phone)/i, /don'?t call (them |him |her )?back/i, /i can'?t talk long/i],
...
export function mockAnalyze(conversation: string): ScamAnalysis {
  ...
  const voiceAuthenticitySignal = /grandson|grandma|grandpa|it'?s me|son|daughter/i.test(conversation)
